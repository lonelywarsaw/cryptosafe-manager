...
- Один таймер буфера в `StateManager`: счётчик в статус-баре («Буфер: N с»), по нулю — очистка буфера ОС.
- Копирование логина, пароля и «Копировать всё» через `ClipboardService` с авто-очисткой по таймауту из настроек.
...
**Реализовано:** журнал аудита с целостностью, просмотр и проверка.

- Каталог `src/core/audit/`: `audit_logger.py`, `log_signer.py` (HKDF `audit-signing` + HMAC-SHA256), `log_verifier.py`, `log_formatters.py`, `integrity.py` (проверка при старте и по кнопке).
- События vault, входа/выхода, буфера → подписанные записи в `audit_log` (JSON в `entry_data`, цепочка `previous_hash`, без секретов в тексте).
- GUI: окно «Журнал» — таблица, фильтр по типу события, кнопки «Проверить целостность» и экспорт JSON/CSV.
- При запуске после разблокировки — выборочная проверка последних записей; при нарушении — предупреждение.
- Ротация: обрезка старых записей при превышении лимита (`audit_max_entries`, по умолчанию 10 000).

**Планируется:** PDF-отчёты, Ed25519, шифрование логов at rest, периодическая фоновая проверка в UI, расширенные категории событий (смена настроек и т.д.).
