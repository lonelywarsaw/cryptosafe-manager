...
**Реализовано (часть — фундамент журнала):**

- Каталог `src/core/audit/`: `audit_logger.py` (контроллер: события → структурированная запись в БД), `log_signer.py` (HKDF от кэшированного ключа шифрования с info `audit-signing`, подпись **HMAC-SHA256**; ключ в БД не пишется), `log_verifier.py` (проверка подписей для списка записей), `log_formatters.py` (заготовки экспорта JSON/CSV).
...
- Поля записи в JSON (`entry_data`): UTC timestamp, `event_type`, `severity`, `user_id`, `source`, `details`, `entry_id`, `sequence_number`, `previous_hash`; грубая редукция подстрок вроде `password=` в деталях → `[REDACTED]`.
...
**Планируется:** просмотр журнала в GUI с фильтрами, верификация цепочки при старте и по расписанию, экспорт (в том числе PDF), ротация логов, при необходимости Ed25519 вместо HMAC, расширение набора событий.
