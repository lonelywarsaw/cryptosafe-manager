...
6. Тесты: `pytest tests/ -v` или с отчётом покрытия: `pytest tests/ --cov=src --cov-report=term-missing`
...
| [Спринт 4](#спринт-4) | Улучшения GUI, импорт/экспорт записей |
...
**Планируется:** реальная работа с ключами.

- Реализация вывода ключа из пароля и соли (например, PBKDF2).
- Реализация store_key / load_key (безопасное хранение/загрузка ключа).
- Интеграция с шифрованием записей (использование выведенного ключа вместо заглушки).

---

### Спринт 3

**Планируется:** замена placeholder-шифрования на стойкое.

- Замена XOR на AES-GCM (или иной рекомендуемый режим) в `core/crypto/`.
- Сохранение интерфейса EncryptionService и зануления чувствительных данных в памяти.

---

### Спринт 4

**Планируется:** расширение возможностей работы с записями и интерфейсом.

- Импорт записей из файла (например, CSV или собственный формат).
- Экспорт записей в файл.
- Улучшения GUI по необходимости (навигация, поиск, сортировка).
