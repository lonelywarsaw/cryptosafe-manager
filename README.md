...
│   │   ├── audit/               # Спринт 5 (часть): подписка на события, подпись, форматирование
...
### Сверка с TRD (`sprints/sprint1.md` … `sprint5.md`)

- **Спринт 1** — каркас MVC, конфиг, SQLite с миграциями, XOR-placeholder и смена на реальное шифрование в спринте 3, шина событий, аудит-таблица, StateManager, базовый GUI и тесты: соответствует целям первого TRD; детали полей схемы со временем эволюционировали (например, `vault_entries` как один BLOB, `key_store` в виде `key_data`).
- **Спринт 2** — Argon2id, PBKDF2, `key_store`, кэш ключа, задержки при неверном пароле, смена мастер-пароля, интеграция с GUI: основные Must-пункты TRD закрыты; интеграция с OS keychain в TRD помечена как Should/Optional и не реализована.
- **Спринт 3** — каталог `core/vault/`, AES-256-GCM per-entry, CRUD, события, таблица с поиском и генератором в диалоге: соответствует ядру TRD; отдельная таблица `deleted_entries` для soft delete в TRD — Should, в проекте soft delete иной/упрощённый по смыслу спринта.
- **Спринт 4** — `core/clipboard/`, автоочистка, адаптеры платформ, события буфера, мониторинг **изменения** содержимого извне, статус в UI: соответствует базовой модели TRD; не доведены до полного TRD, например, детекция «внешнее приложение прочитало буфер» (MON-1) и отдельный индикатор в трее (UI-2).
- **Спринт 5** — по TRD реализованы только **разделы 1–4** (архитектура пакета `audit/`, HMAC-подпись и HKDF отдельно от AES-ключа, структура записи в JSON, расширение `audit_log` и индексы); разделы 5–12 (верификация при старте, GUI viewer, экспорт PDF, ротация, нагрузочные требования и т.д.) в README отмечены как запланированные доработки.
...
| [Спринт 5](#спринт-5) | Аудит с целостностью (в работе: п. 1–4 TRD; viewer и верификация — дальше) |
...
- Глобальное «Показать пароли» на панели и `Ctrl+Shift+P`: учёт выделения строки через `selectionModel().selectedRows()` и `currentRow` (клик по колонке с виджетом пароля больше не «ломает» выбор); при включении без выделения раскрываются все видимые записи; если раскрывать нечего — флажок инструмента сбрасывается.
- По истечении таймера буфера: после очистки через платформенный адаптер дополнительно очищается `QClipboard` **только из GUI-потока** (из фонового `threading.Timer` сервиса вызов Qt не делается — иначе возможны сбои на Windows); при обнулении счётчика в главном окне выполняется вторая очистка Qt на случай, если секрет уже снят фоновым таймером.
- Тест автоочистки буфера (`tests/test_sprint4_clipboard.py`): ожидание очистки поллингом вместо одного жёсткого `sleep`, чтобы тест был устойчивее на медленных средах.
...
**Реализовано (разделы 1–4 technical requirements в `sprints/sprint5.md`, без разделов 5–12):**

- Каталог `src/core/audit/`: `audit_logger.py` (контроллер: события → структурированная запись в БД), `log_signer.py` (HKDF от кэшированного ключа шифрования с info `audit-signing`, подпись **HMAC-SHA256** — как fallback в TRD; ключ в БД не пишется), `log_verifier.py` (проверка подписей для списка записей), `log_formatters.py` (заготовки экспорта JSON/CSV).
- Подписка на шину событий (`EntryCreated` / `Updated` / `Deleted`, `UserLoggedIn` / `Out`, `ClipboardCopied` / `Cleared`) без прямой связи бизнес-модуля с SQL.
- Поля записи в JSON (`entry_data`): UTC timestamp, `event_type`, `severity`, `user_id`, `source`, `details`, `entry_id`, `sequence_number`, `previous_hash`; грубая редукция подстрок вроде `password=` в деталях → `[REDACTED]` (LOG-3 в упрощённом виде).
- Схема БД `audit_log` расширена: `sequence_number`, `previous_hash`, `entry_data` (BLOB), колонка `public_key` зарезервирована под будущий Ed25519; индексы по `sequence_number` и `action`; миграция `user_version` 3→4 и идемпотентное добавление колонок для старых файлов БД (`SCHEMA_VERSION` = 4).

**Планируется дальше по тому же TRD:** просмотр журнала в GUI с фильтрами, верификация цепочки при старте и по расписанию, экспорт (в т.ч. PDF), ротация логов (DB-4), Ed25519 при желании вместо HMAC, остальные категории событий и разделы 5–12.
