...
**GUI (доработки вместе со спринтом 3):** кнопки «Показать/Скрыть» вместо emoji; локализация `Last modified`, `Category`, генератора и URL; исправлен глобальный показ паролей (`selectedRows` / `currentRow`).

**Тесты:** `tests/test_sprint4_clipboard.py` — таймер, очистка, события, GUI smoke (где доступен дисплей).

**Ограничение Windows:** история Win+V может показывать старые фрагменты — это журнал ОС, не текущий буфер после таймаута.
